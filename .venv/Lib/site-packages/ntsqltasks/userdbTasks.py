...
def execute_insert_update_delete_query(connection, sql_query, values):
    """
    Execute an INSERT, UPDATE, or DELETE SQL query with values.
    """
    try:
        # ✅ Debugging: Check if connection is valid
        if not connection:
            print("❌ Database connection is None or invalid")
            return False

        # ✅ Debugging: Print SQL query being executed
        print(f"🛠️ Executing Query: {sql_query} with values: {values}")

        cursor = connection.cursor()  # ✅ FIXED: Do not use 'with connection.cursor() as cursor:'
...
def execute_insert_update_delete_query1(connection, query, values):
    try:
        if not connection:
        ...
        cursor.close()
        return last_id  # Return the correct inserted ID
    except Exception as e:
        print("Database Insert Error:", e)
        return None


def execute_insert_update_delete_query2(connection, query, values):
    try:
        with connection.cursor() as cursor:
            cursor.execute(query, values)
            connection.commit()
            return cursor.lastrowid  # ✅ Return the actual inserted ID
    except Exception as e:
        logging.exception(e)
        connection.rollback()
...
def execute_select_query_vals(connection, sql_query, values):
    try:
        if not connection:
        ...
        if not result:
            print("⚠️ Warning: No data found for query:", sql_query)
            return None  # Prevents 'NoneType' error
...
# def execute_select_query(connection, sql_query, values):
#     try:
#         if not connection:
#             print("❌ Error: Connection is None")
#             return None
...
def execute_select_query(connection, sql_query):
    try:
        if connection is None:
            print("❌ Failed connection: Database connection is None")
            return "Failed connection"
        
        cursor = connection.cursor()  # Explicitly creating a cursor
        cursor.execute(sql_query)
        records = cursor.fetchall()
        cursor.close()
        
        return records  # Returns fetched records from database
...
def insert_User():
    return """
        INSERT INTO User_Info (Username, Email, Password, Status, Has_Edit_Access, Is_Confirmed, Organization_Owner) 
...
def insert_UserOrg():
    return """
        INSERT INTO User_Org_Info (User_ID, Org_ID) 
        VALUES (%s, %s);
    """
...
def create_user(connection, username, email, hashed_password):
    """
    Inserts a new user into the User_Info table and returns the User_ID.
    """
    try:
        if not connection:
        ...
        sql_query = """
            INSERT INTO User_Info (Username, Email, Password, Status,Has_Edit_Access, Is_Confirmed, Organization_Owner)
            VALUES (%s, %s, %s, 'Active',1,1,1);
        """
        
        cursor = connection.cursor()  # Explicitly create the cursor
        cursor.execute(sql_query, (username, email, hashed_password))
        connection.commit()
...
def select_user_by_email_query():
    return "SELECT * FROM User_Info WHERE Email = %s;"

def update_user_password_query():
    return "UPDATE User_Info SET Password = %s WHERE Email = %s;"


def check_password_by_user_id_query():
    return """
        SELECT Password FROM User_Info WHERE User_ID = %s;
    """
...
#def insert_Org():
#    return """INSERT INTO Org_Info (Org_Name, Status) VALUES (%s, %s);"""

#def insert_User():
#    return """INSERT INTO User_Info (Username, Email, Password, Status) VALUES (%s, %s, %s, %s);"""




def select_userID_by_email_query():
    return "SELECT SQL_NO_CACHE User_ID FROM User_Info WHERE Email = %s;"




def insert_org(connection, org_name):
    """
    Inserts a new organization into the Org_Info table and returns the Org_ID.
    """
    try:
        if not connection:
            print("❌ Database connection is not available.")
        ...
        sql_query = """
            INSERT INTO Org_Info (Org_Name, Status)
            VALUES (%s, 'Active');
        """
        ...
        cursor.close()
        return org_id  # Return the new Org_ID
...
def execute_select_query_vals1(connection, sql_query, vals):
    from mysql.connector.errors import Error
